...
// script runs where they haven't been generated (e.g. a standalone deploy of this
// service), regenerating would silently drop every /api/* redirect to the docs
// home. Guard against that: if the docs content looks incomplete, keep the
// committed redirects.json rather than overwriting it with a broken map.
const apiPageCount = [...routes].filter(r =>
  r.startsWith('api-reference/'),
).length;
if (apiPageCount < 200) {
  console.warn(
    `[build-map] docs content looks incomplete: found ${apiPageCount} ` +
      `api-reference pages (expected 400+); the generated TypeDoc pages are ` +
      `probably missing.`,
  );
  if (existsSync(OUT)) {
    console.warn(
