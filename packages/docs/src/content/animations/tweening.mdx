...
[`tween`](/api-reference/core/tweening/functions/tween) function:

In the example above, we animate the x coordinate of our circle from `-300` to
`300` over a span of `2` second.

The [`tween`](/api-reference/core/tweening/functions/tween) function takes two parameters. The first
...
[timing functions](/api-reference/core/tweening).

A timing function takes a number in the range `[0, 1]` and returns another
number in the same range but with a modified rate of change. Revideo
...
Let's use the [`easeInOutCubic`](/api-reference/core/tweening/functions/easeInOutCubic) function to
fix our animation:
...
[`map`](/api-reference/core/tweening/functions/map) function can be used to interpolate between two
...
> [!TIP]
>
> All [complex types](/api-reference/core/types) in Revideo provide a static method
...
functions such as the [`Vector2.arcLerp`](/api-reference/core/types/classes/Vector2#arcLerp). It
...
The [`tween`](/api-reference/core/tweening/functions/tween) function is useful when we need to
orchestrate complex animations. However, there's a better way of tweening
individual properties. You may recall from the guide section that the following
tween:
...
Here, we use a [`SignalTween`](/api-reference/core/signals/interfaces/SignalTween) signature that
...
The [`spring`](/api-reference/core/tweening/functions/spring) function allows us to interpolate
...
The first argument of the [`spring`](/api-reference/core/tweening/functions/spring) function expects
an object that describes the physical properties of our spring. Revideo
...
settled. The generator created by the [`spring`](/api-reference/core/tweening/functions/spring)
...
All nodes provide a [`save`](/api-reference/2d/components/classes/Node#save) method which allows
us to save a snapshot of the node's current state. We can then use the
[`restore`](/api-reference/2d/components/classes/Node#restore) method at a later point in our
...
It is also possible to provide a custom [timing function](/api-reference/core/tweening) to
the [`restore`](/api-reference/2d/components/classes/Node#restore) method.
...
state by invoking the [`save`](/api-reference/2d/components/classes/Node#save) method multiple
times. When calling [`restore`](/api-reference/2d/components/classes/Node#restore), the node will
