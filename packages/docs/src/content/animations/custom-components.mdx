...
Components are classes like [`Rect`](/api-reference/2d/components/classes/Rect) and
[`Circle`](/api-reference/2d/components/classes/Circle) that can abstract rendering and data
...
[`SignalValue<>`](/api-reference/core/signals/type-aliases/SignalValue) as such:
...
[`Node`](/api-reference/2d/components/classes/Node) or one of its subclasses. If you don't want to
...
[`Layout`](/api-reference/2d/components/classes/Layout), you should extend
[`Layout`](/api-reference/2d/components/classes/Layout) and
[`LayoutProps`](/api-reference/2d/components/interfaces/LayoutProps).
...
property with the same name. Revideo provides type decorators to
facilitate this like `@initial()` and `@signal()`. Click [here](/animations/signals) for
...
passed to color signal as Revideo knows that it must be of a
...
[Rect](/api-reference/2d/components/classes/Rect) in the array.
