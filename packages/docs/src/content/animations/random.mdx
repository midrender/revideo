...
unpredictability in your animation. In Revideo, it's achieved using the
[`useRandom()`](/api-reference/core/utils) function. It returns a random number
...
10 (exclusive). Check the [`Random` api](/api-reference/core/scenes/classes/Random) to see all
available methods.
