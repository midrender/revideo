...
For more information, check out the [renderVideo()](/api-reference/renderer/renderVideo)
API reference.
...
which will yield frames that we can export. It is not necessary to understand
how this works exactly, but if you are interested, you can read about the
animation flow in revideo [here](/animations/flow).

### Adding Video and Audio elements

At the start of our generator function, we add [Video](/api-reference/2d/components/classes/Video)
and [Audio](/api-reference/2d/components/classes/Audio) tags to our `view`, which will display
them on the canvas. Other components you could add include
[Txt](/api-reference/2d/components/classes/Txt) or [Img](/api-reference/2d/components/classes/Img) tags, or basic
shapes like [Rect](/api-reference/2d/components/classes/Rect) or
[Circle](/api-reference/2d/components/classes/Circle). You can find the API for all components
[here](/api-reference/2d/components).
...
The function [waitFor](/api-reference/core/flow/functions/waitFor) does - as the name suggests -
...
[Img](/api-reference/2d/components/classes/Img) to our canvas. We set its initial width to only 1%
of the screen, as we want it to grow as the video plays. We also pass a
[reference](/animations/references) through `ref={logoRef}`, which we had initialized
...
The flow of these animations is determined by the keywords [chain](/animations/flow/#chain)
and [all](/animations/flow/#all). The former instructs revideo to play its input animations
...
flow in revideo [here](/animations/flow).
...
Each video in Revideo is represented by a project configuration object. In
...
[scene hierarchy](/animations/hierarchy) section. For now, what's important is that,
in our example, we add an individual [`<Circle/>`](/api-reference/2d/components/classes/Circle)
...
[grab a reference to it](/animations/references). That's the purpose of the
[`createRef`](/api-reference/core/utils/functions/createRef) function. We use it to create a
reference and pass it to our circle using the
[`ref`](/api-reference/2d/components/interfaces/NodeProps#ref) attribute:
...
This [creates a tween animation](/animations/tweening) that smoothly changes the fill
color over one second.

Animations in Revideo don't play on their own, we need to explicitly tell
...
happen at the same time. The [animation flow](/animations/flow) section goes into more
...
[all]: /api-reference/core/flow/functions/all
