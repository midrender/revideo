...
([docs](/api-reference/revideo-ffmpeg/concatenate-media)) and `mergeAudioWithVideo()`
([docs](/api-reference/revideo-ffmpeg/merge-audio-with-video)) functions from `@revideo/ffmpeg`.
...
A string pointing towards your project file. This will probably be
`./src/project.ts`.
...
A `RenderVideoUserProjectSettings` object that overrides the settings defined in
your project file. All properties are optional:
...
Dimensions of the video to render. Uses the value specified in your project by
default.
...
Background color of the video (e.g. `'#000000'`). Uses the value specified in
your project by default.

##### exporter?

The exporter to use. Defaults to the WebAssembly exporter
