...
[`createSignal()`](/api-reference/core/signals/functions/createSignal) function, where the first
...
2. update the value:
   ...
   signal(3);
   ```
3. create a [tween](/animations/tweening) for the value:
   ```ts
   yield * signal(2, 0.3);
   ```
...
computationally heavy operations. In fact, Revideo uses signals internally
...
[`DEFAULT`](/api-reference/core/signals/variables/DEFAULT) symbol to it:
...
We can also use the [`DEFAULT`](/api-reference/core/signals/variables/DEFAULT) symbol for
...
example below, we set the [`lineHeight`](/api-reference/2d/components/classes/Layout#lineHeight)
of the [`Txt`](/api-reference/2d/components/classes/Txt) node to `150%`. This will override its
...
If we want to reset the [`lineHeight`](/api-reference/2d/components/classes/Layout#lineHeight)
back to the default, inherited value, we can do so with
[`DEFAULT`](/api-reference/core/signals/variables/DEFAULT):
