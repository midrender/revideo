...
function ([API reference](/api-reference/renderer/renderVideo)):
...
([API reference](/api-reference/renderer/renderVideo)) provides a
`settings.workers` argument that you can use to parallelize rendering in a single
...
([API reference](/api-reference/renderer/renderPartialVideo)) that you
can use to render a partial video - you can use it together with
`concatenateMedia()` ([API reference](/api-reference/revideo-ffmpeg/concatenate-media)) and
`mergeAudioWithVideo()`
([API reference](/api-reference/revideo-ffmpeg/merge-audio-with-video)) to
