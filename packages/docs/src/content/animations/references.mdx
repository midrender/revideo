...
> In Revideo, the JSX components immediately create and return an instance
...
Each node in Revideo has a property called `ref` that allows you to create
...
[`createRef()`](/api-reference/core/utils/functions/createRef) function. Continuing with our
...
Instead, it's a [signal-like](/animations/signals) function that can be used to access it.
Invoking it without any arguments (`circle()`) returns our instance.
...
We can use the [`makeRef()`](/api-reference/core/utils/functions/makeRef) function to simplify this
process:
...
You can also use the [`createRefArray()`](/api-reference/core/utils/functions/createRefArray) helper
...
> [!TIP]
>
> Check out [the looping section in the flow guide](/animations/flow#looping) to see how an
> array of references can be used to orchestrate animations.
>
...
tedious. The [`createRefMap()`](/api-reference/core/utils/functions/createRefMap) helper function
...
We can use [`makeRefs()`](/api-reference/core/utils/functions/makeRefs) to eliminate this
