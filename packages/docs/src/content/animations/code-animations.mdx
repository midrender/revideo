...
The [`Code`](/api-reference/2d/components/classes/Code) node is used for displaying code snippets.
...
The code to display is set via the [`code`](/api-reference/2d/components/classes/Code#code)
...
called [`CODE`](/api-reference/2d/code/functions/CODE). It allows the `Code` node to parse the
...
created using [`Code.createSignal()`](/api-reference/2d/components/classes/Code#createSignal).
While the generic [`createSignal()`](/api-reference/core/signals/functions/createSignal) would work
...
snippet, you can use the [`append`](/api-reference/2d/code/classes/CodeSignalContext#append) and
[`prepend`](/api-reference/2d/code/classes/CodeSignalContext#prepend) methods. They can either
...
[`insert`](/api-reference/2d/code/classes/CodeSignalContext#insert),
[`replace`](/api-reference/2d/code/classes/CodeSignalContext#replace), and
[`remove`](/api-reference/2d/code/classes/CodeSignalContext#remove) to modify the code at specific
...
The [`edit`](/api-reference/2d/code/classes/CodeSignalContext#edit) method offers a different way
of defining code transitions. It's used together with the
[`replace`](/api-reference/2d/code/functions/insert), [`insert`](/api-reference/2d/code/functions/insert), and
[`remove`](/api-reference/2d/code/functions/remove) helper functions that are inserted into the
...
the ones created by
[`Code.createSignal()`](/api-reference/2d/components/classes/Code#createSignal). This means that
...
A [`CodeRange`](/api-reference/2d/code/type-aliases/CodeRange) is used to specify a continuous span of
...
For convenience, the [`word`](/api-reference/2d/code/functions/word) and
[`lines`](/api-reference/2d/code/functions/lines) helper functions are provided to create some of
...
[`findFirstRange`](/api-reference/2d/components/classes/Code#findFirstRange),
[`findAllRanges`](/api-reference/2d/components/classes/Code#findAllRanges), and
[`findLastRange`](/api-reference/2d/components/classes/Code#findLastRange) methods to find the
...
The [`selection`](/api-reference/2d/code/classes/CodeSignalContext#selection) property can be used
...
[`getPointBBox`](/api-reference/2d/components/classes/Code#getPointBBox) and
[`getSelectionBBox`](/api-reference/2d/components/classes/Code#getSelectionBBox) can be used to
retrieve the position and size of a specific character or a range of characters,
respectively. The returned value is a [`bounding box`](/api-reference/core/types/classes/BBox) in
...
[`DefaultHighlightStyle`](/api-reference/2d/code/variables/DefaultHighlightStyle) is used. You can
...
[`highlighter`](/api-reference/2d/components/classes/Code#highlighter) property. This will
...
You can use the [`withDefaults`](/api-reference/2d/utils) helper function to
